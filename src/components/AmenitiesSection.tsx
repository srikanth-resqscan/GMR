  ...
  return (
    <div id="amenities_wrapper" className="space-y-20">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-2xl shadow-slate-100/50 overflow-hidden grid grid-cols-1 lg:grid-cols-12">
        <div className="lg:col-span-5 bg-gradient-to-br from-blue-950 via-indigo-950 to-violet-950 p-8 sm:p-12 text-white flex flex-col justify-between relative">
          <div className="absolute inset-0 bg-grid-pattern opacity-10 pointer-events-none"></div>
          <div className="relative z-10">
            <span className="inline-block px-3 py-1 bg-white/10 text-blue-200 border border-white/15 rounded-md text-[11px] font-bold uppercase tracking-widest mb-6">
              Grand Clubhouse
            </span>
            <h3 className="text-3xl sm:text-4xl font-light leading-tight tracking-tight">
              16,000 Sft of <br />
              <span className="font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-violet-300">Pure Luxury</span>
            </h3>
            <p className="text-sm text-blue-200/90 mt-5 leading-relaxed font-sans">
        ...
        <div className="lg:col-span-7 p-6 sm:p-10 bg-slate-50/40 relative">
          ...
          <div className="relative z-10">
            <h4 className="font-sans font-bold text-lg text-slate-950 mb-6 flex items-center space-x-2">
              <Award className="h-5 w-5 text-blue-600 shrink-0 animate-pulse" />
            ...
            <div className="space-y-4">
              {CLUBHOUSE_AMENITIES.map((level, i) => (
                <div
                  key={i}
                  className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 hover:border-blue-200 transition-colors duration-300"
                >
                  <div className="flex items-center space-x-3.5 shrink-0">
                    <div className="h-9 w-9 bg-blue-50 text-blue-700 font-mono text-xs font-bold rounded-lg flex items-center justify-center border border-blue-100">
                      L{i}
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2 sm:justify-end">
                    {level.items.map((item, itemIdx) => (
                      <span
                        key={itemIdx}
                        className="px-3 py-1 bg-blue-50/50 text-blue-950 border border-blue-100/60 text-[11px] font-semibold rounded-md"
                      >
                        {item}
                      </span>
                    ))}
      ...
      <div>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 border-b border-slate-100 pb-5">
          <div>
            <h3 className="text-2xl sm:text-3xl font-light text-slate-900 tracking-tight leading-tight">
              Campus Facilities & <span className="font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-violet-600">Active Arenas</span>
            </h3>
              ...
              <button
                ...
                className={`px-4.5 py-2 rounded-md text-xs font-semibold uppercase tracking-wider transition-all duration-300 focus:outline-none cursor-pointer ${
                  activeCategory === cat.id
                    ? "bg-gradient-to-r from-blue-600 to-violet-600 text-white shadow-md font-bold"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {filteredAmenities.map((amenity, idx) => (
            <div
              key={idx}
              className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm flex items-center space-x-3.5 hover:shadow-2xl hover:shadow-slate-100/60 hover:border-blue-100 hover:scale-[1.01] transition-all duration-300"
            >
              <div className="p-2.5 bg-blue-50 text-blue-700 rounded-lg shrink-0 border border-blue-100/50">
